...
class Auth
{
    ...
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
        if(Db::name('user')->where(['username' => $username, 'merchant_id' => $extend['merchant_id']])->find()){
            $this->setError('Username already exist');
            return false;
        }

        if(Db::name('user')->where(['email' => $email, 'merchant_id' => $extend['merchant_id']])->find()){
            $this->setError('邮箱已被使用');
            return false;
        }
